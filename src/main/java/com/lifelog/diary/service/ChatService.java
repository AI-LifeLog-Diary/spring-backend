...
public class ChatService {
    ...
    public Flux<String> chatAndCreateDiary(String conversation, String currentDiary, User user) {
        ...
        Flux<String> followUpFlux = followQuestionService
                .streamFollowUpQuestionFromConversation(conversation)
                .map(token -> "FOLLOWUP: " + token);

        Flux<String> diaryFlux = gptService
                .streamDiaryFromConversation(conversation)
                .map(token -> "DIARY: " + token);
